...
%             as the result is smoothed. 使用高斯核函数对数据进行非参数估计（nonparametric estimation） 的回归模型
%
%Outputs:
...
if useparfor
    numWorkers = Inf;  %inf: 无穷大
else
    numWorkers = 1;
end

if size(data, 2) < size(data, 1)  % size(data, 1)矩阵行数；size(data, 2)矩阵列数
    data = data.';   %1*60000
end
...
if size(data, 1) == 1
    data = makesegments(data, max(Tau)); %程序下方定义makesegments: 将数据data分段，分成 max(Tau)+1 组（6*60000）
end
...
x = linspace(L, R, bins + 1); % x is the bins 左 borders.生成 bins + 1 个点。这些点的间距为 (R-L)/bin
dx = x(2) - x(1);
C = x + dx / 2;
C = C(1:end - 1); % C is the bin centers.
[N, ~] = histcounts(dat(1, :), x); % 将数据dat分布到 50 个等间距 bin 内. N is the amount of data in each bin（x的每个区间中的数据个数）.
...
if strcmp(method, 'Nadaraya-Watson') == 0  % strcmp(s1, s2): 比较字符串 s1 和 s2是否相同
    for tau = Tau
        for i = 1:bins % Here, I use Matlab parallel computing toolbox. If you do not have it just use 'for loop' not 'parfor loop'
            ...
            x1 = x;
            I = dat1(1, :) >= x1(i) & dat1(1, :) < x1(i + 1);  %在bin中的数据的位置
    ...
    %         h = 1.05 *(length(dat(1, :)))^(-1/ 5) * std(dat(1, :)); % 带宽如何选择？
    %         K = @(x)(x>-sqrt(1) & x<sqrt(1)).*3 /4 .*(1-x.^2); % Epanechnikov kernel
%     h = 1.05 *(length(dat(1, :)))^( -1/ 4) * std(dat(1, :));
%     K = @(x)(x>-sqrt(1) & x<sqrt(1)).*3 /4 .*(1-x.^2); % Epanechnikov kernel
% 第一组数据
...
% 第二组数据
    ...
    for tau = Tau
        for i = 1:bins    % 并行 for 循环 ：Here, I use Matlab parallel computing toolbox. If you do not have it just use 'for loop' not 'parfor loop'.
            dat1 = dat;
            A = K((dat1(1, :) - C(i))./ h);  % 注意力权重
...
%preselect fit methods for efficiency重新选择拟合方法以提高效率
%fit = fittype( 'poly1' );  fitlm: 拟合线性回归模型
...
for i = 1:bins
    ...
    D1(i) = fit1.Coefficients.Estimate(2);  % 拟合 Tau 和 M(i),取斜率
    
    % 估计扩散项
    fit2 = fitlm(Tau1, A2(:, i) - (D1(i) .* Tau1').^2, 'weight', WeightM2(:, i));
    ErrorD2(i) = fit2.Coefficients.SE(2);
    D2(i) = 1 / 2 * fit2.Coefficients.Estimate(2);  % 斜率
...
%we use this for bootstrapping（估计标准误差，置信区间）
%
% example:
% makesegments(1:10,5)
%
