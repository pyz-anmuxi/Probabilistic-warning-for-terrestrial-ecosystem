function [D1, D2, D4, ErrorD1, ErrorD2, N, C] = LangevinReconst(data, L, R, bins, Tau, dt, method, title, useparfor)
    ...
    % Outliers（异常值） should be replaced by NAN on beforehand.
    ...
    if strcmp(method, 'Nadaraya-Watson') == 0  % strcmp(s1, s2): 比较字符串 s1 和 s2是否相同
        ...
        h = 1.048 * (length(dat(1, :)))^(-1 / 5) * std(dat(1, :)); % std:标准差
        K = @(x)(x>-sqrt(5)&x<sqrt(5)).*3 .*sqrt(5)./100 .*(5-x.^2); % Epanechnikov kernel
        ...
        for tau = Tau
            for i = 1:bins    % 并行 for 循环 ：Here, I use Matlab parallel computing toolbox. If you do not have it just use 'for loop' not 'parfor loop'.
                ...
                A = A(ndx);
                B = B(ndx);
                N1 = sum(A .* B);   %非参数核函数估计第一条件矩
                N2 = sum(A .* B.^2);  % 估计第二条件矩
                N4 = sum(A .* B.^4);  % 估计第四条件矩
                E = sum(A);
                M1(i) = N1 / E;  % 标准化？
                M2(i) = N2 / E;
                M4(i) = N4 / E;
