function res = Langevin_bootstrap(data, reconstResult, bootmethod, nreplicates, boot_options)
    %Langevin_bootstrap - bootstrap methods of LangevinReconstr
    ...
    %
    %Usage: Langevin_bootstrap(data,reconstResult,bootmethod,nreplicates,boot_options)
    ...
    %   MaxStep - the stepsize for the Euler Maruyama solver (default
    %             0.001)
...
%%%% 四种bootmethod
%%%% 'error-propagation', 'model-based', 'replicates'
    if any(strcmpi(bootmethod, {'error-propagation', 'model-based', 'replicates'}))
        ...
        % parentmod：基于重构结果得到的Langevin方程
        eqns = parentmod.equilibria;
        
    %%%%%%%%如果为'model-based', 'replicates'，则parentmod改为realmod,使用原模型生成数据
        if strcmpi(bootmethod, 'model-based') || strcmpi(bootmethod, 'replicates')
            ...
            if isfield(boot_options, 'model') % 如果为'model-based',则parentmod改为realmod,使用原模型生成数据
                if ~isa(boot_options.model, 'langevin_eq')
                    error('LangevinReconst_boot:wrongmodel', 'The model should be a langevin_eq object');
                end
                parentmod = boot_options.model;  % boot_options.model指realmod
                eqns = parentmod.equilibria;
            else  % 如果为 'replicates'，则使用原模型生成的数据采样
                parentmod = [];
...
%%%%%%%%%%%%% 对每组数据重构，并计算 pdfs 和 mean exit time 
            ...
            for i = 1:n1
                if i + k <= size(bootstrapdatay, 2)
                    ...
                    try
                        ...
                        % res_boot包括reconstr，results，haserror
                ...
                fprintf('--------------Get results of the %dth set of bootstrap data--------------\n', i);
            end  % tmpbootstrap中储存n1组重构及计算结果
...
%%%%%%%%%%%%%% block
        ...
        for i = 1:nreplicates
            ...
            ndxes{i} = ndx;
            fprintf('--------------Getresults of the %dth set of bootstrap data--------------\n', i);
        end
...
function res = removebias(results, bootmethod, parentmodel)
    ...
    %        results: [1*100 struct]
    %       bootdata: [15001*200 double]
