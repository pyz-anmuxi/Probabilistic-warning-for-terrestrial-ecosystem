        ...
        function res = getinitfun(obj, atype, varargin)
            ...
            switch atype
                case 'normalize'
                    fun = varargin{1};
                    domain_ = obj.domain;
                    sumpdf = integral(fun, domain_(1), domain_(2));   % 函数fun在区间domain_(1), domain_(2)上的积分
                    res = @(x)fun(x)/sumpdf;
                case 'normal_dom'
...
% pdepe解向前F-K方程，用于pdf函数和runpdf函数
        function result = solvepdf(obj, pdf0, varargin)
            ...
            x = linspace(L, R, options.nx);
               % Mesh = x;
            if isnumeric(pdf0) && length(pdf0) == 1  % isnumeric: 确定输入是否为数值数组
                pdf0 = obj.getinitfun('dirichlet', pdf0, x);
                if isa(obj.D1, 'chebfun')
                    pdf0 = chebfun(pdf0, obj.domain, 'vectorize');
                end
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%% 设置pdepe中的方程函数fp_equat
            if isempty(obj.DD2)
                DD2_ = diff_fun(obj.D2);  % diff_fun：对函数D2求导
            else
                DD2_ = obj.DD2;
            end
            %we bind（绑定） the D1 D2 and DD2 functions to this anonymous function（匿名函数）
            if isa(obj.D1, 'chebfun')
...
% 确定输入Dfun为函数
        function [res, isconstant] = set_Dfun(obj, Dfun, avar)  % 输入D1=Dfun
            if nargin < 3
                avar = '';
            end
            isconstant = false;
            ...
            % 当输入函数为数字时
            elseif isnumeric(Dfun) && numel(Dfun) == 1  % 计算数组中满足条件的个数 %设置D2和DD2
                isconstant = true;
                if strcmp(avar, 'g')
            ...
            % 当输入函数为函数句柄时, 
            elseif isa(Dfun, 'function_handle')  % %设置D1
                try
                    if strcmp(avar, 'g')  % strcmp：比较字符串
                        Dfun = @(x)0.5*Dfun(x).^2;
                    end
                    %test whether the function is in vector notation or
                    %returning a constant.
                    s1 = Dfun(1:10);   % D1(x) 共10个
                    if numel(s1) == 1   % 计算数组的元素个数
                        %constant
                        isconstant = true;
                        res = @(x)Dfun(x)+zeros(size(x));
                    elseif numel(s1) ~= 10  % ~=：非
    ...
    methods
        function obj = langevin_eq(varargin)  % varargin：可变长度输入参数列表
            ...
            if nargin > 0   % 判断输入变量的个数
                ndx = cellfun(@ischar, varargin);   % cellfun：对元胞数组中的每个元胞应用函数ischar
                options = varargin(ndx);  % 提取字符串
                
%%%%%%%%%%%%%%%%%%%%%%%%%% 重构后
                if nargin > 0 && isfield(varargin{1}, 'ErrorD2') %  isfield(结构体,字段)：判断字段是否属于结构体 
                    ...
                    if isfield(results, 'bootstrap')
                        ...
                        results = rmfield(results, 'bootstrap');  % 删除结构体变量的字段
                    end
                    ...
                    if ((nargin == 1 && exist('fittype', 'file')) || any(strcmp(options, 'weightedspline')))  % exist: 检测im中的变量是否存在  any: 检测矩阵中是否有非零元素
                        ...
                        ft = fittype( 'smoothingspline' );   % fittype: 非线性拟合
                        %
                   %%%%%%%%%%%%%拟合重构后的D1
                        if all(results.ErrorD1 == 0)   % all: 检测矩阵中是否全为零元素
                            w1 = ones(size(results.ErrorD1));
                        else
                            w1 = 1 ./ results.ErrorD1;
                        end
                        [xData, yData, weights1] = prepareCurveData(results.C, results.D1, w1 );  % 把要拟合的数据格式统一
                        opts = fitoptions( 'Method', 'SmoothingSpline', 'SmoothingParam', 0.9982070110205727, 'Weights', weights1);  % fitoptions: 拟合选项的设置
                    ...
                    %%%%%%%%%%%%% 拟合重构后的D2
                        ...
                        if exist('chebfun', 'file') && ~any(strcmp(options, 'nocheb'))
                            ...
                            D1 = chebfun(d1(l), [L1 R1], 'equi', 5000);  % 返回函数定积分
                            D2 = chebfun(d2(l), [L1 R1], 'equi', 5000);
                            obj.set('D1', D1, 'D2', D2);  % Inf×1 chebfun赋值给D1和D2
                        else
                    ...
                    elseif nargin == 1 || any(strcmp(options, 'spline')) %default if curvefitting toolbox is missing
                        d1 = griddedInterpolant(results.C, results.D1, 'spline');  % 网格数据集进行插值
                ...
                % openrealmod：设置面向对象obj=langevin_eq 
                else
                    obj.set(varargin{:}); % 调用set函数,将输入的varargin设置为obj的属性
                end
                % 设置平衡点信息
...
% 计算平衡点、稳定性、边界条件、domain
        function equilibria1 = find_equilibria(obj, mode)
            ...
            if strncmpi(mode, 'd', 1)  %比较字符串的前 n 个字符（不区分大小写）
                %naively D1
                d1 = obj.D1;
                mode = 'deterministic';
            else
            ...
            Eqs = roots_fun(d1, obj.domain);   % 求解平衡点
            DD1 = diff_fun(d1);   % 导函数：DD1 = @(x)(afun(x+dt/2)-afun(x-dt/2))/dt  (雅可比矩阵)
            isstable = DD1(Eqs) < 0;   % 判断稳定性，稳定：1；不稳定：0
            ...
            for i = 1:length(Eqms)
                if isstable(i)
                    BC = 'RR';
                    dom = [-Inf Inf];
                    % 设置右盆地[Eqms(2).x Eqms(3).x]
                    if i > 1
                        BC(1) = 'A';
                        dom(1) = Eqms(i - 1).x;
                    end
                    % 设置左盆地[Eqms(1).x Eqms(2).x]
                    if i < length(Eqms)
                        BC(2) = 'A';
                        dom(2) = Eqms(i + 1).x;
                    end
...
% 创建ode函数
        function res = odefun(obj, dt)
            ...
            d1 = griddedInterpolant(Mesh, obj.D1(Mesh), 'linear');  % 网格数组插值
...
% 求解SDE，生成时间序列数据
        function [res, fcn] = simulate(obj, tspan, y0, options)
            ...
            if nargin < 4
                options = odeset([], 'AbsTol', []);  % 为 ODE 和 PDE 求解器创建或修改 options 结构体
            end
            ...
            options = odeset(options);  % 为ODE和PDE求解器修改或创建options结构体
            options.MaxStep = dt;
...
% 从稳态概率分布中提取随机数作为初始条件
        function res = draw_pdf(obj, siz1, siz2)
            %draw_pdf - draw（提取）random number from the stationary pdf
            ...
            if ~isfield(obj.xtra, 'results') || ~isfield(obj.xtra.results, 'pdf')
                thepdf = obj.pdf;  %两种方法数值解稳态概率分布
            else
                thepdf = obj.xtra.results.pdf;
            end
            %make cumulative density function
            cdf = cumtrapz(thepdf.x, thepdf.PDF);  % 累积梯形数值积分:根据x标量间距对PDF积分
            if nargin < 2
                siz1 = 1;
            end
            if nargin < 3
            ...
            res = interp1(cdf, thepdf.x, rand(siz1, siz2));  % interp1：使用线性插值返回一维函数在特定查询点的插入值。
        end
...
% bvp45计算exit_time(边值问题)
        function res = mean_exit(obj, ibasin, PDF, varargin)
            ...
            elseif nargin == 4
                ...
                options = bvpset;  % 创建或更改边界值问题的 options 结构体
            end
            
            % 分别对每个盆地计算obj.mean_exit(i, PDF);
            if ischar(ibasin) && strcmp(ibasin, 'all')  % 比较两个字符串
                res = cell(1, obj.nbasin);   % obj.nbasin:计算吸引盆地个数，即稳定平衡点个数 
                for i = 1:length(res)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 设置边界条件           
            ...
            if isstruct(ibasin)
                ...
                stableeq = find([obj.equilibria.stable]);  % find：返回非零元素在向量或者矩阵中的位置
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% The BVP solver returns the structure 'sol'. 
                ...
                try  % 执行语句并捕获产生的错误
                    ...
                    Tx = deval(sol, xpoints);  % 对微分方程数值解的结构体进行插值
                catch err
                    disp(err.message);  %  显示变量 X 的值，而不打印变量名称。
                    Tx = NaN;
                end
                res.T = Tx(1, :);
                if ~isempty(PDF)
...
% 方法一：This code (numerically) solves the Fokker-Planck equation.
            % 
            ...
            if nargin > 3
                options = struct(varargin{:});
            elseif nargin == 3
                options = varargin{1};  %varargin 就是一个输入变量，它可以是任意个不定个数的输入参数
            else
                options = [];
            end
            if ~isfield(options, 'maxtime')  % 检查结构体options是否包含由'maxtime'指定的域，
            ...
            % pdepe解向前F-K方程
            result = obj.solvepdf(pdf0, options);  % 方程的解
            result.PDF = result.pdfs(end, :);  % 稳态
            
% 方法二：another way of determining the PDF using the effective potential
            %integral D1/D2:
            %
            %
            if isa(obj.D1, 'chebfun')  % 判断obj.D1是否为class_name类型
                try    % 执行语句并捕获产生的错误
                    h2 = 1 / obj.D2 * exp(cumsum(obj.D1 / obj.D2));
                    
                    %normalize to surface = 1
                    xpoints = linspace(obj.domain(1), obj.domain(2), 4000);
                    intg = trapz(xpoints, h2(xpoints));  % 通过梯形法计算h2(xpoints)的近似积分
                    %normalized pdf
                ...
                h1 = @(x)obj.D1(x)./obj.D2(x);
                h = integralx_fun(h1, obj.domain(1), obj.domain(2));   % int(D1/D2,x) 积分
                %
                ...
                %normalize to surface = 1
                xpoints = linspace(obj.domain(1), obj.domain(2), 4000);
                intg = trapz(xpoints, h2(xpoints));  % trapz：梯形数值积分
                %normalized pdf
...
% pdepe求解survival概率
        function res = survival(obj, ibasin, varargin)
            ...
            %    + additional fields:
            %    time_range = range of time （时间范围，纵轴ylimit）
            %    ntime = number of points for output of time axis(纵轴时间的离散，决定了曲线的光滑程度)
            %    nx = number of points in x direction
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%% 设置pdepe函数中的options：time_range，ntime，nx
            if nargin > 3
                options = struct(varargin{:});
            elseif nargin == 3
            ...
            if ~isfield(options, 'ntime') % (纵轴时间的离散，决定了曲线的光滑程度)
                options.ntime = 500;
            end
            if ~isfield(options, 'nx')
                options.nx = obj.nx;   % 属性中设置 nx=100
            end
 
            % for循环，分别对每个盆地计算存活概率
            if ischar(ibasin) && strcmp(ibasin, 'all')
                res = cell(1, obj.nbasin);  % obj.nbasin: 吸引盆地的数量，稳定点个数
                for i = 1:length(res)  
                    res{i} = obj.survival(i, options);  % 将计算结果存到res；2*
                end
            ...
            if isstruct(ibasin)
                ...
                nbasin1 = obj.nbasin;  % obj.nbasin：计算吸引盆地个数  
                if ibasin > nbasin1
                    error('langevin_eq:survival', 'basin number too large');
                end
                stableeq = find([obj.equilibria.stable]); % 返回数组中非零元素的索引量
                eqn = obj.equilibria(stableeq(ibasin)); % 得到每个稳定平衡点信息：值，BC，domain
            end
            
            % 保证区间在obj.domain内
            domain_ = eqn.domain;
            domain_(domain_ < obj.domain(1)) = obj.domain(1);  % 左边界 < domain时
            domain_(domain_ > obj.domain(2)) = obj.domain(2);  % 右边界 > domain时
            
            % 设置pdepe函数中的BC
            switch eqn.BC  % 根据不同条件控制不同操作
            ...
            %设置pdepe函数中的方程函数
            %we bind(绑定) the functions D1 D2 and DD2_ in the anonymous(匿名) function
            if isa(obj.D1, 'chebfun')
            ...
            if isempty(options.time_range)
                ...
                options.time_range = [0 find(meansurv<0.2,1)+1];
            end
...
% 设置obj = langevin_eq的属性
        function set(obj, varargin)
            % 将数组cell型转换为struct类型（字段）
            if nargin > 1
...
% plot exit_time\survice\pst\Ueff等
...
% 画 mean_exit
            function hax1 = plot_mean_exit(T, domain, WT, acolor, nx)
                if isa(T, 'chebfun')  % 1000组数据
                    l1 = linspace(min(domain), max(domain), nx);
                    hax1 = plot(l1, T(l1), [acolor '-'], 'LineWidth', 1.8, 'Color', [137/256 23/256 23/256]);
                else
                    l1 = linspace(min(domain), max(domain), numel(T));
                    hax1 = plot(l1, T, [acolor '-'], 'LineWidth', 1.8, 'Color', [251/256 197/256 107/256]);  
                end
                hold on
                if ~isempty(WT)
                    plot([l1(1), l1(end)], [WT, WT], 'Color', acolor, 'LineWidth', 2.5,'LineStyle', '--');
                end
            end
            ...
            hax1 = [];
            res = [];
            % 判断输入变量是否为字符串char还是元胞数组cell还是结构体struct（Fig2直接用模型得到结构）
            if nargin == 1
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            elseif iscell(varargin{1})  
                for i = 1:length(varargin{1})
                    hax1(i) = obj.plot(varargin{1}{i}, varargin{2:end});  % 将输入变量改为一个盆地的数据进行plot
                end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% plot初值给定情况
                ...
                switch varargin{1}
                    case {'D1', 'D2' , 'DD2', 'D4'}
                        ...
                        if isfield(obj.xtra, 'bootstrap') && ~args.noboot
                            ...
                            fill([results.C , fliplr(results.C)], args.timescale * [range(1, :), fliplr(range(2, :))], [196 165 222]./256, 'EdgeColor', 'none');
                        elseif ~(~isfield(obj.xtra, 'reconstr') || isempty(obj.xtra.reconstr)) && (~isfield(args, 'error') || args.error)
                            ...
                            elseif strcmp(varargin{1}, 'D2')
                                fill([results.C , fliplr(results.C)], [args.timescale * results.D2 - results.ErrorD2, fliplr(args.timescale * results.D2 + results.ErrorD2)], [196 165 222]./256, 'EdgeColor', 'none')
                            end
                        end
                        hax1 = plot(dom, args.timescale .* x(dom), 'Color',[156/256 39/256 176/256], 'linewidth', 1.5, 'tag', varargin{1}); % 绿色
                        xlim(obj.domain);
                        xlabel(obj.namex);
                        ylabel(args.ylabel)
                    ...
                    case {'survival_func', 'exit_distrib'} %args1 is x0 args2 = survival results
                        ...
                        if iscell(results)
                            ...
                            if x0 > min(results.x) && x0 < max(results.x)  % 判断给定初值在哪个吸引盆地
                                n = find(results.x >= x0, 1);  % 找到大于给定初始状态的位置
                                surv = results.survival(:, n(1));   % 给定初始状态的存活概率
                                res.surv = surv;
                                res.t = results.t;
                    ...
                    case 'mean_exit'
                        ...
                        if isfield(obj.xtra, 'bootstrap')
                            ...
                            fill([res.C , fliplr(res.C)], [res.CL_Ts(1, :), fliplr(res.CL_Ts(2, :))], [196 165 222]./256, 'EdgeColor', 'none');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            else % varargin{1}为struct，包含某个盆地计算得到的survival、exit time：100*500数据
                
                result = varargin{1};
                args = struct(varargin{2:end});
                
   % 判断varargin{1}中包含哪种数据'T'、eff、
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% % mean exit time             
                if isfield(result, 'T') || isfield(result, 'T_left') 
                    ...
                    hax1 = plot_mean_exit(result.T, result.domain, result.WT, [156/256 204/256 101/256], obj.nx);  % 调用plot_mean_exit
                    xlim(obj.domain);
                    yl = get(gca, 'ylim');
                    if yl(1) < 0
                        set(gca, 'ylim', [0 yl(2)]);
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% % effective potential
                elseif isfield(result, 'ueff') 
                    ...
                    hax1 = fill([result.dom, result.dom(end), result.dom(1)], [U, 0, 0], [196 165 222]./256);
                    xlim(obj.domain);
                    xlabel(obj.namex);
                ...
                elseif isfield(result, 'PDF') || isfield(result, 'pdfs')  
                    ...
                    if (it == length(result.t)) && isfield(result, 'PDF')
                        if updatefig
                            ...
                            % 重构一次
                            hax1 = fill([result.x result.x(end) result.x(1)], [result.PDF, 0 0], [196 165 222]./256);
                        end
                    else
                        if updatefig
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% % survival    
                elseif isfield(result, 'survival') 
                    ...
                    colormap( flipud([linspace(102, 236, 100)', linspace(77, 232, 100)', linspace(142, 242, 100)']./255)); % 绿色
                    [~, hax1] = contourf(x, y, result.survival, 100);% 填充二维等高线，100:等高线数目
                    set(hax1, 'LineColor', 'none')
                    hold on
                    [~, hax1] = contour(x, y, result.survival, [0.5 0.5]);  % 画指定的等高线
...
function [tout, yout] = euler(odefunct, tspan, y0, options)
    ...
    if nargin < 4
        ...
        nonNegative = odeget(options, 'NonNegative', [], 'fast');  % 提取 ODE 选项值
    end
    ...
    step_tspan = median(diff(tspan));   % 数组的中位数值
    ...
    if nargout > 0
        ...
        nout = 1;
        tout(nout) = t;
        yout(nout, :) = transpose(y);  % 调换矩阵的行列索引值
    end
    %
    %MAIN LOOP
...
function y = myode45(h, timespan, t0, x0)
    if length(timespan) == 1
        ...
        [~, y] = ode45(h, timespan(:), x0);
        if numel(y) < numel(timespan)   %n返回数组中元素个数
...
function fun = diff_fun(afun, dt)
    ...
    if isa(afun, 'chebfun')
        fun = diff(afun);  % 函数对x求导
    elseif isa(afun, 'function_handle')   % 函数句柄
...
function rootsx = roots_fun(afun, dom)
    if isa(afun, 'chebfun')
        ...
        ndx = diff(sign(res)) ~= 0;    % diff：计算相邻元素之间的差分
        rootsx = x(ndx);   % 前后符号相反则存在零根
        % adapt with precision
        for i = length(rootsx): -1:1
            if isnan(res(i))  %确定哪些数组元素为 NaN
                rootsx(i) = [];
            else
                rootsx(i) = fzero(afun, rootsx(i));  % fzero：查找一元连续函数的零点
            end
        end
    end
end
